...
if( !class_exists( 'vxg_zoho_pages' ) ) {
...
* @since       1.0.0
*/
class vxg_zoho_pages   extends vxg_zoho{
    public $ajax=false;
/**
...
require_once(self::$path . 'templates/crm-entry-box.php'); 
  ...
  */
  ...
  if(isset($_GET[$this->id."_logs"]) && current_user_can($this->id.'_read_settings')){
      $msg=__('Error While Clearing Zoho Logs','gravity-forms-zoho-crm');
      $level="error";
      if(!empty($_GET[$this->id."_logs"])){
      $msg=__('Zoho Logs Cleared Successfully','gravity-forms-zoho-crm');   
      $level="updated";
      }
  ...
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'gravity-forms-zoho-crm') . '</a>' );
        }
        return $links;
   }
  ...
  /**
    ...
    public function add_form_settings_menu( $tabs, $form_id ) {
        ...
        $tabs[] = array( 'name' => $this->id, 'label' => esc_html__("Zoho", 'gravity-forms-zoho-crm') , 'query' => array( 'fid' => null),'icon' => 'dashicons-cloud dashicons' );
...
$this->push($entry,$form,'add_note');
  ...
  */
  ...
  $menus[] = array("name" => $this->id, "label" => esc_html__('Zoho','gravity-forms-zoho-crm'), "callback" =>  array($this, "mapping_page"), "permission" => $this->id.'_read_feeds');
  ...
  */
  public  function setup(){
      ...
      RGForms::add_settings_page(array('name' => $this->id,'tab_label' => esc_html__('Zoho','gravity-forms-zoho-crm'),'icon' => 'dashicons-cloud dashicons',"handler"=>array($this, "settings_page")));
  ...
  $msg=__('You do not have permissions to export logs','gravity-forms-zoho-crm');
  ...
  $fields=array(); $field_titles=array("#",__('Status','gravity-forms-zoho-crm'),__('Zoho ID','gravity-forms-zoho-crm') ,__('Entry ID','gravity-forms-zoho-crm'),__('Description','gravity-forms-zoho-crm'),__('Time','gravity-forms-zoho-crm'));
  ...
  $msg=__('You do not have permissions to clear logs','gravity-forms-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  ...
  */
  ...
  esc_html_e('You do not have permissions to access this page','gravity-forms-zoho-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'gravity-forms-zoho-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'gravity-forms-zoho-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','gravity-forms-zoho-crm'),$count));  
    }
  ...
  if(is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','gravity-forms-zoho-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','gravity-forms-zoho-crm'),"delete"=>__('Delete','gravity-forms-zoho-crm'),
  'send_to_crm_bulk'=>__('Send to Zoho','gravity-forms-zoho-crm'),'send_to_crm_bulk_force'=>__('Force Send to Zoho - Ignore filters','gravity-forms-zoho-crm'));
  $base_url=$this->get_base_url();
  $option=get_option($this->id.'_meta',array());
$objects=isset($option['objects']) && is_array($option['objects']) ? $option['objects'] : array();
      $statuses=array("1"=>__("Created",'gravity-forms-zoho-crm'),"2"=>__("Updated",'gravity-forms-zoho-crm'),"error"=>__("Failed",'gravity-forms-zoho-crm'),"4"=>__("Filtered",'gravity-forms-zoho-crm'),"5"=>__("Deleted",'gravity-forms-zoho-crm')); 
...
public function get_menu_links($current_page=""){
  ...
  $feeds_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview={$this->id}&tab=feed&id={$id}" );
      ...
      $menu_links=array(
  'settings'=> array( 
  "title"=>__('Zoho Settings','gravity-forms-zoho-crm'),
  "link"=>$settings_link,
  "current"=>$current_page == 'settings' ? true : false
  ),
  'feed'=> array( 
  "title"=>__('Zoho Feeds','gravity-forms-zoho-crm'),
  "link"=>$feeds_link,
   "current"=>$current_page == 'feed' ? true : false
  ),
  'log'=> array( 
  "title"=>__('Zoho Log','gravity-forms-zoho-crm'),
  "link"=>$logs_link,
...
$skipped_fields[$k]=$v; continue;
 ...
 if(  in_array($v['label'],array('Product Details') )  && in_array($module,array('SalesOrders','PurchaseOrders','Contacts'))){ 
    $show_account=true;
...
$skipped_fields[$k]=$v;  continue;
      ...
      if(in_array($k,$req_fields) || !empty($v['req'])){
   $map_fields[$k]=$v;       
      } 
...
//
  ...
  $sel_fields=array(""=>__("Standard Field",'gravity-forms-zoho-crm'),"value"=>__("Custom Value",'gravity-forms-zoho-crm'));
...
include_once(self::$path . "templates/fields-mapping.php"); 
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Zoho','gravity-forms-zoho-crm');
        return $plugins;
    }
  /**
  ...
  */
  ...
  $msg=__("Please Choose Object",'gravity-forms-zoho-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'gravity-forms-zoho-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'gravity-forms-zoho-crm');
  }
  if($msg !=""){
  ?>
  <div class="error below-h2" style="background: #f3f3f3">
...
$this->field_map_object($account,$feed,$info);
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_camp']=$this->gen_select($camps,$status_sel,__('Select Campaign','gravity-forms-zoho-crm'));
  ...
  case"refresh_users":
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','gravity-forms-zoho-crm'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
  ...
  $res['data']=$data;   
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_layout']=$this->gen_select($users,'',__('Select Layout','gravity-forms-zoho-crm'));
    $meta['layouts']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','gravity-forms-zoho-crm');
    }
}
  die(json_encode($res));    
  }
...
if(isset($_REQUEST[$this->id.'_tab_action']) && $_REQUEST[$this->id.'_tab_action']=="get_token"){
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','gravity-forms-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $id=$this->post('id');
...
$res=$data->del_account($id);
 if($res){
       $msg=__('Account Deleted Successfully','gravity-forms-zoho-crm');
  $msg_arr=array('msg'=>$msg,'class'=>'updated');   
 }else{
       $msg=__('Error While Removing Account','gravity-forms-zoho-crm');
...
$field_options="<option>".__("Select Object",'gravity-forms-zoho-crm')."</option>"; 
  if(is_array($objects)){
...
return $arr;
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','gravity-forms-zoho-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Gravity Forms Zoho Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'gravity-forms-zoho-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to save settings','gravity-forms-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
$msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','gravity-forms-zoho-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','gravity-forms-zoho-crm'));
  }
...
$client=$api->client_info();
  ...
  check_admin_referer('vx_nonce');
  if(!current_user_can($this->id."_read_settings")){ 
  $msg=__('You do not have permissions to refresh lists','gravity-forms-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $msgs['refresh']=array("class"=>"updated","msg"=>__('Successfully Refreshed Picklists','gravity-forms-zoho-crm')); 
...
if(!empty($info['data'])){
 ...
 if(isset($_POST['vx_test_connection'])){
  $msg=__('Connection to Zoho is Working','gravity-forms-zoho-crm');
  
  if($conn_class != "updated" ){
      $msg=__('Connection to Zoho is NOT Working','gravity-forms-zoho-crm');  
  }
  $title=__('Test Connection: ','gravity-forms-zoho-crm');
...
$sel2_css=$base_url. 'css/select2.min.css';
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','gravity-forms-zoho-crm'); 
  return;
  }
  //
...
if(!empty($_POST['account'])){
 ...
 if(isset($info['meta']['object']) && isset($feed_update['object']) && $feed_update['object'] == $info['meta']['object']){
...
$msg=sprintf(__("Feed Updated. %sback to list%s", 'gravity-forms-zoho-crm'), '<a href="'.$feed_link.'">', "</a>");
  }
  else{
$msg=__("Feed could not be updated. Please enter all required information below.", 'gravity-forms-zoho-crm');
$class='error';
  }
  if(!empty($msg)){
...
include_once(self::$path . "templates/feed-account.php");
  ...
  */
  ...
  $desc.=" Web2".$row['object'];
   ...
   $desc=sprintf(__('%s Filtered','gravity-forms-zoho-crm'),$row['object']);   
  }else   if($row['status'] == 5){
   $desc=sprintf(__('%s Deleted','gravity-forms-zoho-crm'),$row['object']);  
  }else{
  $desc.=$row['object'];
  }
  }else{
  ...
  $title=__("Failed",'gravity-forms-zoho-crm');   
  if( $row['status'] == 1){
  $title=__("Created",'gravity-forms-zoho-crm');   
  }else if($row['status'] == 2){
  $title=__("Updated",'gravity-forms-zoho-crm');   
  }else if($row['status'] == 4){
  $title=__("Filtered",'gravity-forms-zoho-crm');   
  }else if($row['status'] == 5){
  $title=__("Deleted",'gravity-forms-zoho-crm');   
  }
  $row['_crm_id']= $crm_id;
  $row['a_link']=$link;
  $row['desc']=$desc;
  ...
  */
  ...
  if(RGFormsModel::get_input_type($field) == "address") {
      $fields[] =  array($field["id"], GFCommon::get_label($field) . " (" . _x("Full" , 'Full field label', 'gravity-forms-zoho-crm') . ")");
  }
  ...
  $fields[]=array('id',__('Entry ID','gravity-forms-zoho-crm'));
  $fields[]=array('form_id',__('Form ID','gravity-forms-zoho-crm'));
  $fields[]=array('entry_url',__('Entry URL','gravity-forms-zoho-crm'));
  $fields[]=array('date_created',__('Entry Date','gravity-forms-zoho-crm'));
  $fields[]=array('ip',__('User IP','gravity-forms-zoho-crm'));
  $fields[]=array('source_url',__('Source Url','gravity-forms-zoho-crm'));
  $fields[]=array('form_title',__('Form Title','gravity-forms-zoho-crm'));
  $fields[]=array('status',__('Entry Status','gravity-forms-zoho-crm'));
  $this->fields=array('gf'=>array("title"=>__('Gravity Forms Fields','gravity-forms-zoho-crm'),"fields"=>$fields)); 
  ...
  $fields['feeds']=array("title"=>__('ID from other Feeds','gravity-forms-zoho-crm'),"fields"=>$feeds);
  }
  $this->fields=$fields;
  }
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $msg=__( 'Successfully Connected to Zoho','gravity-forms-zoho-crm' );
  ...
  if(!empty($info['access_token']) && empty($info['error'])){
      $info['error']='API Token is Not Valid';
  }
  if(!empty($info['error'])){ 
  $info['msg']=$info['error']; 
  $info['class']="error"; 
